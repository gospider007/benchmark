...
func tempTest() {
	...
	svr := server.RequestServer(ctx, "127.0.0.1:3334")
	defer svr.Close()
	// log.Print("FhttpRequest 1k time: ", tools.TestMain(request.WangluozheRequest, 100000, "http://127.0.0.1:3334/1k", 10)) //10并发
	...
	log.Print("GospiderRequest 1k time: ", tools.TestMain(request.GospiderRequest, 100000, "http://127.0.0.1:3334/1k")) //1 并发
}
func main() {
	// tempTest()
	oneThreadTest()
